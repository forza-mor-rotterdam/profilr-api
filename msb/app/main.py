...
def authenticate_user(authorization_header: str):
    ...
    if (
        ...
        and auth_parts[1] == os.getenv("MSB_USER_TOKEN")
    ):
        return True
    return False
...
async def detail(item_id, authorization: Union[str, None] = Header(default=None)):
    ...
    melding_dict = {
        m.get("detail").get("result").get("id"): m.get("detail")
        for m in meldingen()
        if m.get("detail")
    }
...
async def mutatieregels(
    item_id, authorization: Union[str, None] = Header(default=None)
):
    user = authenticate_user(authorization)
    if not user:
        raise HTTPException(
    ...
    melding_dict = {
        m.get("detail").get("result").get("id"): m.get("mutatieregels")
        for m in meldingen()
        if m.get("mutatieregels")
    }
    melding = melding_dict.get(int(item_id))
    if not melding:
        return JSONResponse(status_code=404, content={"message": "Item not found"})
