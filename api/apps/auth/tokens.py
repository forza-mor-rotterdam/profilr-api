...
class JWTAccessToken:
    ...
    def decode_token(token=None, missing_key=False):
        ...
        try:
            jwt = JWT(
                jwt=token, key=get_keyset(), algs=settings["ALLOWED_SIGNING_ALGORITHMS"]
            )
        except JWTExpired:
            raise AuthenticationFailed(
                "API auth problem: token expired {}".format(token)
            )
        except InvalidJWSSignature as e:
            raise AuthenticationFailed(
                "API auth problem: invalid signature. {}".format(e)
            )
    ...
    def decode_claims(raw_claims):
        ...
        for user_id_field in settings["USER_ID_FIELDS"]:
            ...
            if user_id:
                try:
                    validate_email(
                        user_id
                    )  # Signalen uses email addresses as usernames
        ...
        return (
            claims,
            user_id,
        )  # only user_id is used, SIA contains its own authorization model
