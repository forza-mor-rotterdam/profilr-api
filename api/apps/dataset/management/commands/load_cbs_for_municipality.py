...
class Command(BaseCommand):
    ...
    def _municipal_code_is_correct(self, municipal_code):
        ...
        area_type = AreaType.objects.get(
            code=REQUIRED_DATASETS["CBS_MUNICIPAL_BORDERS_DATASET"]
        )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Municipal code (CBS GM_CODE).",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not self._municipal_code_is_correct(municipal_code):
            raise CommandError(
                "Provided municipal code is not present in CBS data, exiting!"
            )
        self.stdout.write("Required CBS data are available.")

        # Datatype for our newly loaded data, note we drop the old copy.
        wijk_dataset_code, buurt_dataset_code = self._derive_dataset_codes(
            municipal_code
        )
        wijk_area_type, _ = AreaType.objects.get_or_create(
            code=wijk_dataset_code, name=wijk_dataset_code
        )
        buurt_area_type, _ = AreaType.objects.get_or_create(
            code=buurt_dataset_code, name=buurt_dataset_code
        )
        ...
        with transaction.atomic():
            ...
            self.stdout.write(
                f"There are {to_remove_b.count()} old (buurt) area instances to remove"
            )
            ...
            municipality_area_type = AreaType.objects.get(
                code=REQUIRED_DATASETS["CBS_MUNICIPAL_BORDERS_DATASET"]
            )
            municipality = Area.objects.get(
                _type=municipality_area_type, code=municipal_code
            )
            self.stdout.write(
                f'\nNeighborhoods for the municipality of "{municipality.name}" will be copied'
            )

            to_copy_w = Area.objects.filter(
                _type__code=cbs_wijk_dataset, geometry__intersects=municipality.geometry
            )
            to_copy_b = Area.objects.filter(
                _type__code=cbs_buurt_dataset,
                geometry__intersects=municipality.geometry,
            )

            self.stdout.write(
                f'There are {to_copy_w.count()} new "wijk" area instances to copy.'
            )
            self.stdout.write(
                f'"Wijk" area instances will be copied to dataset/AreaType {wijk_area_type.name}.'
            )
            self.stdout.write(
                f'There are {to_copy_b.count()} new "buurt area instances to copy.'
            )
            self.stdout.write(
                f'"Buurt" area instances will be copied to dataset/AreaType {buurt_area_type.name}.'
            )
