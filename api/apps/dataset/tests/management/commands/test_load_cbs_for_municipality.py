...
from apps.dataset.management.commands.load_cbs_for_municipality import REQUIRED_DATASETS
from apps.locations.factories import AreaFactory, AreaTypeFactory
...
class TestLoadCBSForMunicipality(TestCase):
    def setUp(self):
        ...
        self.area_type_m = AreaTypeFactory.create(
            code=self.area_type_code_m, name=self.area_type_code_m
        )
        self.area_type_w = AreaTypeFactory.create(
            code=self.area_type_code_w, name=self.area_type_code_w
        )
        self.area_type_b = AreaTypeFactory.create(
            code=self.area_type_code_b, name=self.area_type_code_b
        )
    ...
    def test_load_wijk_and_buurt(self):
        ...
        buffer = StringIO()
        call_command(
            "load_cbs_for_municipality", "--municipal_code=GM0363", stdout=buffer
        )
        ...
        self.assertEqual(
            Area.objects.filter(_type__code="gm0363-wijk").count(), 1
        )  # one "wijk" in Amsterdam
        self.assertEqual(
            Area.objects.filter(_type__code="gm0363-buurt").count(), 1
        )  # one "buurt" in Amsterdam
    ...
    def test_municipal_code_does_not_exist(self):
        ...
        e = cm.exception
        self.assertIn(
            str(e), "Provided municipal code is not present in CBS data, exiting!"
        )
